...
import { updateCategory } from "@/lib/supabase/categories-actions";
...
const categoryEditSchema = z.object({
  ...
  color: z
    ...
    .nullable(),
  icon: z.string().min(1, "Select an icon.").nullable(),
});

export function EditCategory({ category }: CategoryActionsProps) {
  ...
  const handleEdit = async (values: CategoryEditFormValues) => {
    ...
    const payload: Partial<InsertCategory> = {
      name: values.name,
    ...
    await handleActionToast(updateCategory(category.id, payload), {
      form,
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription
                    className="pt-1"
                    style={{ color: field.value ? String(field.value) : undefined }}
                  >
